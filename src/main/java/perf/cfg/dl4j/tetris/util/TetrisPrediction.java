...
public class TetrisPrediction {
	...
	public TetrisPrediction(TetrisDataGetter tetrisDataGetter, TetrisDataGetter nextCubeGetter,
			TetrisScoreCalculator calculator) {
	...
	public LinkedList<CubeActionType> predictNextAction() {
		byte[] tmp = currStep;
		currStep = lastStep;
		lastStep = tmp;
		...
		LinkedList<CubeActionType> bestRoute = findRouteByBestStatus();
		if (bestRoute == null) {
		...
		for (int i = bestRoute.size() - 1; i >= 0; i--) {
			CubeActionType c = bestRoute.get(i);
			if (c == CubeActionType.ACTION_MOVE_DOWN) {
				yBias++;
				if (i < bestRoute.size() - 1) {
					CubeActionType next = bestRoute.get(i + 1);
					if (next == CubeActionType.ACTION_MOVE_LEFT
							&& !checkValid(bestTetrisStatus.bestStatus, width, height, bestTetrisStatus.cubeIndexes,
									xBias - 1, yBias)) {
						bestRoute.set(i, CubeActionType.ACTION_MOVE_LEFT);
					} else if (next == CubeActionType.ACTION_MOVE_RIGHT
							&& !checkValid(bestTetrisStatus.bestStatus, width, height, bestTetrisStatus.cubeIndexes,
									xBias + 1, yBias)) {
						bestRoute.set(i, CubeActionType.ACTION_MOVE_RIGHT);
					}
				}
			} else if (c == CubeActionType.ACTION_MOVE_LEFT) {
				xBias++;
			} else if (c == CubeActionType.ACTION_MOVE_RIGHT) {
				xBias--;
			}
		}
		return bestRoute;
	...
	private int getAvgXLoc(int[] indexes) {
		int sum = 0;
		for (int idx : indexes) {
			sum += idx % width;
		}
	...
	private LinkedList<CubeActionType> transformList(int transformNum) {
		LinkedList<CubeActionType> ret = new LinkedList<CubeActionType>();
		for (int i = 0; i < transformNum; i++) {
			ret.add(CubeActionType.ACTION_TRANSFORM);
		}
		int srcAvg = getAvgXLoc(stepCubeList.get(0));
		int dstAvg = getAvgXLoc(bestTetrisStatus.cubeIndexes);
		for (int i = 0; i < Math.abs(srcAvg - dstAvg); i++) {
			ret.add(srcAvg < dstAvg ? CubeActionType.ACTION_MOVE_RIGHT : CubeActionType.ACTION_MOVE_LEFT);
		}
		if (srcAvg == 0) {
			ret.addFirst(CubeActionType.ACTION_MOVE_RIGHT);
		} else if (srcAvg == width - 1) {
			ret.addFirst(CubeActionType.ACTION_MOVE_LEFT);
		}
		return ret;
	}

	private LinkedList<CubeActionType> findRouteByBestStatus() {
		int srcIndexes[] = stepCubeList.get(0);
		int dstIndexes[] = bestTetrisStatus.cubeIndexes;
		if (dstIndexes == null)
			return null;
		for (int idx : dstIndexes) {
			if (bestTetrisStatus.bestStatus[idx] != 0)
				return null;
		}
		...
		if (bestTetrisStatus.turnNum > 0) {
			LinkedList<CubeActionType> ret = transformList(bestTetrisStatus.turnNum);
			bestTetrisStatus.turnNum = 0;
			return ret;
		}
		Integer xBias = null, yBias = null;
		if (srcIndexes.length != dstIndexes.length || srcIndexes.length == 0)
			return null;
		...
		return getRoute(bestTetrisStatus.bestStatus, srcIndexes, xBias.intValue(), yBias.intValue());
	}

	private LinkedList<CubeActionType> findRouteByNewBestStatus() {
	...
	private boolean checkIndexValid(int[] indexes) {
		...
		for (Cube possibleCube : Common.possibleCubeList()) {
			...
			if (cubeIndexLen == indexes.length)
				return true;
		}
		return false;
	}
		...
		for (int i = left; i <= right; i++) {
			for (int j = bot; j <= 0; j++) {
				if (checkValid(scoreStepTmp[stepDepth], width, height, indexes, i, j)) {
					...
					LinkedList<CubeActionType> route = getRoute(scoreStepTmp[stepDepth], indexes, i, j);
					if (route == null)
						continue;
	...
	private LinkedList<CubeActionType> getRoute(byte[] tetrisData, int[] indexes, int xBias, int yBias) {
		LinkedList<CubeActionType> ret = new LinkedList<CubeActionType>();
		return doGetRoute(tetrisData, width, height, indexes, xBias, yBias, xBias, yBias, ret, 0) ? ret
				: null;
	}

	private static boolean doGetRoute(byte[] tetrisData, int width, int height,
			int[] indexes, int xBias, int yBias, int lastXBias, int lastYBias, LinkedList<CubeActionType> list, int depth) {
		if (xBias == 0 && yBias == 0) {
			return true;
		}
		...
		if (depth >= height * 2)
			return false;
		if (yBias < 0) {
			if (doGetRoute(tetrisData, width, height, indexes, xBias, yBias + 1, xBias, yBias, list,
					depth + 1)) {
				list.add(CubeActionType.ACTION_MOVE_DOWN);
				return true;
			}
		}
		int firstDirection = xBias > 0 ? -1 : 1;
		if (lastXBias != xBias + firstDirection || lastYBias != yBias) {
			if (doGetRoute(tetrisData, width, height, indexes, xBias + firstDirection, yBias, xBias,
					yBias, list, depth + 1)) {
				list.add(firstDirection == 1 ? CubeActionType.ACTION_MOVE_LEFT : CubeActionType.ACTION_MOVE_RIGHT);
				return true;
			}
		}
		if (lastXBias != xBias - firstDirection || lastYBias != yBias) {
	...
	private static boolean checkValid(byte[] tetrisArr, int width, int height, int[] indexes, int xBias, int yBias) {
		for (int i : indexes) {
			int x = i % width;
			int y = i / width;
...
class RouteAndScore {
	LinkedList<CubeActionType> route;
